...
set_samplenum = [ 20000];
ini_num =10;
Pl_inf = zeros(size(set_samplenum,2),ini_num );
%% Five chains
...
for s = 1: size(set_samplenum,2)
    ...
    for in =1:ini_num
        ...
        for i = 1:iter_num
            %% generate one sample
            [X1 ] = gener_sample(domain_counts,unknown_num,unknown_ind,variable_names,dag,X1); 
            [X2 ] = gener_sample(domain_counts,unknown_num,unknown_ind,variable_names,dag,X2); 
            [X3 ] = gener_sample(domain_counts,unknown_num,unknown_ind,variable_names,dag,X3); 
            [X4 ] = gener_sample(domain_counts,unknown_num,unknown_ind,variable_names,dag,X4); 
            [X5 ] = gener_sample(domain_counts,unknown_num,unknown_ind,variable_names,dag,X5);  
        ...
        Pl_inf(s,in ) = Infer;
        display(['The inference is' num2str(Pl_inf(s,in )) ', when initial index is ' num2str(in)]);  
    end 
end
         
plot(1:10 ,Pl_inf(1,:) ,'-*','lineWidth',2);
hold on; plot(1:10 ,Pl_inf(2,:) ,'-.','lineWidth',2);
plot(1:10 ,Pl_inf(3,:) ,'-o','lineWidth',2); 
legend('sample num is 5000', 'sample num is 50000','sample num is 100000')
xlabel('Initialization Index')
